...
#
# OR
# module load gcc/9.2.0 r/4.0.3
# f=/gpfs/fs1/home/t/tpaus/jshinb/ukbb_LDL.D_gwas/scripts/R/get_data_for_GWAS.r
# Rscript $f
...
#==============================================================================#
# select individuals with just brain data and with just metabolite data
#==============================================================================#
eid_common = intersect(d1$eid,d2$eid);length(eid_common)#9129
d1 = subset(d1, !eid %in% eid_common);print(dim(d1))#28,744
d2 = subset(d2, !eid %in% eid_common);print(dim(d2))#108,892
...
d1 = subset(d1, !eid %in% excl.dementia.stroke);print(dim(d1))# 28,480 (-294)
d2 = subset(d2, !eid %in% excl.dementia.stroke);print(dim(d2))# 108,892 (-0)

#==============================================================================#
# select white British individuals: n=7866
#==============================================================================#
d1 = subset(d1, eid %in% incl);print(dim(d1))#23991 (-4489)
d2 = subset(d2, eid %in% incl);print(dim(d2))#90732 (-18160)
...
# brain data ------------------------------------------------------------------
ind = kinship.info$ID1 %in% d1$eid & kinship.info$ID2 %in% d1$eid;print(sum(ind))
while(any(ind))
{
  kinship.anal = subset(kinship.info,ind)
  print(dim(kinship.anal))#4810
  
  # construct kinship matrix
  tmp.fam.ids = sort(unique(c(kinship.anal$ID1,kinship.anal$ID2)))
  print(length(tmp.fam.ids))#8599
  mat = matrix(NA,nrow=length(tmp.fam.ids),ncol=length(tmp.fam.ids))
  rownames(mat) <- colnames(mat) <- tmp.fam.ids
  for(i in 1:nrow(kinship.anal)){
    mat[as.character(kinship.anal$ID1[i]),as.character(kinship.anal$ID2[i])] <- 
      mat[as.character(kinship.anal$ID2[i]),as.character(kinship.anal$ID1[i])] <- 
      kinship.anal$Kinship[i]
    cat("*",sep='')
  }
  #
  nrel = c()
  ...
  remove.ids = unlist(fam.ids)[!unlist(fam.ids)%in% include.ids]#n=463
  d1 = subset(d1,!eid %in% remove.ids);print(dim(d1))#7805
  ind = kinship.info$ID1 %in% d1$eid & kinship.info$ID2 %in% d1$eid;print(sum(ind))
}
brain_data = d1;print(nrow(brain_data))#23528 (-463)

#-------------------------------------------------------------------------------
ind = kinship.info$ID1 %in% d2$eid & kinship.info$ID2 %in% d2$eid;print(sum(ind))#4810
while(any(ind))
{
  kinship.anal = subset(kinship.info,ind)
  print(dim(kinship.anal))#4810
  ...
  remove.ids = unlist(fam.ids)[!unlist(fam.ids)%in% include.ids]#n=463
  d2 = subset(d2,!eid %in% remove.ids);print(dim(d2))#7805
  ind = kinship.info$ID1 %in% d2$eid & kinship.info$ID2 %in% d2$eid;print(sum(ind))
}
metabo_data = d2;print(dim(metabo_data))#86199 (-4533)
rm(d1,d2)
...
# write files ------------------------------------------------------------------
write_tsv(brain_data,"../data/gwas_brain_data.tsv")
write_tsv(metabo_data,"../data/gwas_metabo_data.tsv")
write_tsv(covdata,"../data/gwas_ukb_covdata.tsv")
