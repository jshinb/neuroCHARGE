...
fit_get_results <- function(modName,group){
  # Feb-03-2021: revised to run Sex-by-metabolite interaction model
  ...
  for(yi in 1:length(outcome_vec)){
    ...
    for(xi in 1:length(metaboID_ordered)){
      ...
      if(fit_avail1){
        if(modName %in% c("M1.INTsex","M2.INTsex","M2.HTN.INTsex","M2.HTN.DM.INTsex")){
          if(!'sex1' %in% names(coef(fit_res[[yi]][[xi]]))){
            mycoef = NA
          }else{
            mycoef = coef(fit_res[[yi]][[xi]])[["x:sex1"]]
          }
        }else if(modName %in% c("M1.INTstatin","M2.INTstatin","M2.HTN.INTstatin","M2.HTN.DM.INTstatin")){
          if(!'statin_use1' %in% names(coef(fit_res[[yi]][[xi]]))){
            mycoef = NA
          }else{
            mycoef = coef(fit_res[[yi]][[xi]])[["x:statin_use1"]]
          }
        }else{# no interaction
          mycoef = coef(fit_res[[yi]][[xi]])[["x"]]
        }
        fit_avail2 = !is.na(mycoef)
        if(fit_avail2){ 
          if(modName %in% c("M1.INTsex","M2.INTsex","M2.HTN.INTsex","M2.HTN.DM.INTsex")){
            res= rbind(res,summary(fit_res[[yi]][[xi]])$coef["x:sex1",])
          }else if(modName %in% c("M1.INTstatin","M2.INTstatin","M2.HTN.INTstatin","M2.HTN.DM.INTstatin")){
            res= rbind(res,summary(fit_res[[yi]][[xi]])$coef["x:statin_use1",])
          }
          else{
