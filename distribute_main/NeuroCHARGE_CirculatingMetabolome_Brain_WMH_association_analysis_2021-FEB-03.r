...
# --------------------------------------
WorkingDirectory = "/Users/jshin/OneDrive\ -\ SickKids/neuroCHARGE_BrainMS_CircMetabolism/distribute_working"

# --------------------------------------
# Set working directory
# --------------------------------------
setwd(WorkingDirectory)
input_specification_file = 'Example_cohort_specific_inputs 2.txt'
# input_specification_file = 'Example_cohort_specific_inputs_wi_HTN.txt'
...
cov_columns <- c(age,sex,years,fasting_duration,statin_use,other_lipid_lowering_med_use,BMI,current_smoking,eGFR,hypertension)
cov_columns_names <- unlist(str_split("age,sex,years,fasting_duration,statin_use,other_lipid_lowering_med_use,BMI,current_smoking,eGFR,hypertension",","))
...
for(metaboi in 1:length(platforms)){
  ...
  covs = names(cov_vec)[!names(cov_vec)%in%c('age','sex','other_lipid_lowering_med_use',additional.covs,add.hypertension,cohort_specific_covs)]
  ...
  mod.list <- list(M1=M1,
                   ...
                   M2.HTN.INTstatin=M2.HTN.INTstatin)#B
  print(mod.list)
